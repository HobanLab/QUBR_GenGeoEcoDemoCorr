...
slopes_simulations <- function (Population) {
  ...
  return(list(SD_slopes = SD_slopes, SD_p_values = SD_p_values))
  
  write.csv(SD_slopes, "./data/SD_slopes.csv", row.names = TRUE)
  write.csv(SD_p_values, "./data/SD_p_values.csv", row.names = TRUE)
}

slopes_simulations("SD")

SD_slopes <- read.csv("./data/SD_slopes.csv", row.names = 1)
SD_p_values <- read.csv("./data/SD_p_values.csv", row.names = 1)

load("SD_matrices.RData")
