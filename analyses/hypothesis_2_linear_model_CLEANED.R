  ...
  for (focal_tree in focal_tree_dataframe$focal_tree_row){ #for each focal tree 
    ...
    if(nrow(neighbor_trees_in_buffer) == 0){
      ...
      neighbor_trees_in_buffer_competiton_calc <-  neighbor_trees_in_buffer %>% 
        mutate(focal_distance = as.numeric(st_distance(geometry, focal_tree_in_buffer$geometry, by_element = T))) %>% #calculate the distance between the focal tree and each tree that neighbors 
...
LM_focal_tree_dataframe_no_SCA_outliers <- LM_focal_tree_dataframe[-c(21),]
...
LM_focal_tree_dataframe_no_LCA_outliers <- LM_focal_tree_dataframe[-c(27),]
...
#plotting the linear model in ggplot for SCA
ggplot(data = LM_focal_tree_dataframe, (aes(x=LA_over_distance, y=Canopy_area)))+ 
  geom_smooth(method='glm')+
  geom_point()+
...
#removing outliers based on which points were deemed influential
LM_focal_tree_dataframe_no_CA_outliers <- LM_focal_tree_dataframe[-c(27),]
...
#removing outliers based on which points were deemed influential
LM_focal_tree_dataframe_no_CS_outliers <- LM_focal_tree_dataframe[-c(27),]
...
LC_focal_tree_dataframe <- as.data.frame(LC_focal_tree_dataframe) #focal tree data
...
#removing outliers based on which points were deemed influential
LC_focal_tree_dataframe_no_SCA_outliers <- LC_focal_tree_dataframe[-c(3),]
...
#removing outliers based on which points were deemed influential
LC_focal_tree_dataframe_no_LCA_outliers <- LC_focal_tree_dataframe[-c(3),]
...
#removing outliers based on which points were deemed influential
LC_focal_tree_dataframe_no_CA_outliers <- LC_focal_tree_dataframe[-c(3),]
...
#removing outliers based on which points were deemed influential
LC_focal_tree_dataframe_no_CS_outliers <- LC_focal_tree_dataframe[-c(3),]
...
#removing outliers based on which points were deemed influential
SD_focal_tree_dataframe_no_SCA_outliers <- SD_focal_tree_dataframe[-c(3, 24),]
...
#checking normality of residuals with a histogram and qqnorm plot
ggplot(SD_focal_tree_dataframe, aes(x= SD_gls_focal_SCA$residuals))+
  geom_histogram()+
  labs(title = "Distribution of Residuals for Short Canopy Axis vs. SCA over Distance")+
  xlab("Residuals")+
  ylab("Frequency")
...
#checking equal variance
ggplot(data = SD_focal_tree_dataframe , aes(x = SD_gls_focal_SCA$fitted, y = SD_gls_focal_SCA_gaus$residuals))+
  geom_point()+
  geom_abline(intercept = 0, slope = 0)+
  xlab("Fitted Values")+
  ylab("Residuals")+
...
#removing outliers based on which points were deemed influential
SD_focal_tree_dataframe_no_LCA_outliers <- SD_focal_tree_dataframe[-c(14, 23),]
...
#removing outliers based on which points were deemed influential
SD_focal_tree_dataframe_no_CA_outliers <- SD_focal_tree_dataframe[-c(22),]
...
#checking equal variance
ggplot(data = SD_focal_tree_dataframe, aes(x = SD_gls_focal_CA$fitted, y = SD_gls_focal_CA$residuals))+
  geom_point()+
  geom_abline(intercept = 0, slope = 0)+
  xlab("Fitted Values")+
  ylab("Residuals")+
...
#removing outliers based on which points were deemed influential
SD_focal_tree_dataframe_no_CS_outliers <- SD_focal_tree_dataframe[-c(22, 28),]
...
#removing outliers based on which points were deemed influential
SD_focal_tree_dataframe_no_DBH_outliers <- SD_focal_tree_dataframe[-c(22),]
...
#checking equal variance
ggplot(data = SD_focal_tree_dataframe , aes(x = SD_gls_focal_DBH$fitted, y = SD_gls_focal_DBH$residuals))+
  geom_point()+
  geom_abline(intercept = 0, slope = 0)+
  xlab("Fitted Values")+
  ylab("Residuals")+
