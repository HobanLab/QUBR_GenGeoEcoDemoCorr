...
####NOTE FROM ASH:I DONT THINK THIS CODE WORKS RIGHT NOW CORRECTLY BC THE ST_CONTAINS NEEDS TO BE THE CROPPED RASTER AND THEN 
#ALL OF THE TREE DATA FOR THE ROW CALLS TO WORK CORRECTLY#####
...
fligner <- fligner.test(clay.content.0.5 ~ Locality, data = fixed_field_data_processed_trees_soils)
...
car::leveneTest(fixed_field_data_processed_trees_soils$clay.content.0.5 ~ fixed_field_data_processed_trees_soils$Locality)
